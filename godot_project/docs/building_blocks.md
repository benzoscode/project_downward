...
### relic_pedestal.tscn 遗物台

- **用途**：地面石台之上悬浮展示道具/宝石（上下浮动 + 发光），玩家触碰即拾取（2026-08-23 需求，取代宝箱）。
- **参数**：`item`（lamp/boots/whistle/key/gem_*）；`bob_amplitude`（浮动幅度）；`bob_speed`（浮动速度）。拾取即 `GameState.acquire_item` 并自毁。

### gem_key_gate.tscn 宝石钥匙门

- **用途**：靠近检测玩家是否持有翡翠/琥珀/紫金三颗宝石，集齐即开启（不消耗、不嵌入），否则不开；开门后保持。区别于 gem_gate（镶嵌消耗）。
- **参数**：`gate_id`（持久化 ID，开门状态跨复活保留）。E 交互触发。

### 告示牌（sign）改动（2026-08-23）

- 改为**靠近显示文本、离开消失**（世界内 Label，字号 6），不再按 E 弹 HUD。`sign_texture` 可选木牌/石碑。
